<template>
  ...
  <main class="min-h-screen bg-[#FAF9F6] pt-28 md:pt-36 pb-24">
    <div class="max-w-[1600px] mx-auto px-5 sm:px-8 md:px-12 lg:px-16">
      ...
      <!-- Collections -->
      <section
        v-for="(cat, index) in catalogData"
        :key="cat.name"
        :id="slugify(cat.name)"
      ...
      >
        ...
        <div class="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-4 xl:grid-cols-5 2xl:grid-cols-6 gap-x-5 sm:gap-x-6 md:gap-x-8 gap-y-10 md:gap-y-16">
          <article
            ...
            @keydown.enter.space.prevent="navigateToCheckout(product, cat)"
            role="button"
  ...
  <div class="absolute inset-0 bg-stone-900/0 group-hover:bg-stone-900/5 transition-colors duration-500"></div>
              ...
              <!-- Overlay with elegant reveal -->
              <div class="absolute inset-0 bg-gradient-to-t from-stone-900/60 via-stone-900/20 to-transparent opacity-0 group-hover:opacity-100 transition-opacity duration-700 flex items-end justify-center pb-8 md:pb-10">
                <span class="text-white text-[10px] md:text-[11px] uppercase tracking-[0.45em] font-black border-b border-white/50 pb-1.5 opacity-90 group-hover:opacity-100 translate-y-4 group-hover:translate-y-0 transition-all duration-700">
                  Personalize
                </span>
              </div>
            </div>

            <h3 class="mt-4 md:mt-5 text-[11px] md:text-xs uppercase tracking-[0.3em] font-bold text-stone-800 text-center">
              {{ product.title }}
            </h3>
...
onMounted(() => {
  ...
  }, 6000) // slightly slower for more calm feel
})
...
/* Global smooth scroll (can be moved to global styles if preferred) */
html {
