<template>
  <nav :class="[
    ...
    <div class="flex-1">
      <a href="#home" class="relative block w-36 h-12 md:w-56 md:h-20 perspective-1000 transition-all duration-300">
        <transition name="logo-flip" mode="out-in">
          <img v-if="currentLogo === 1" key="logo1" src="/logo-1.png" alt="Saifee Creations"
            class="w-full h-full object-contain" />
          <img v-else key="logo2" src="/logo-2.png" alt="Saifee Creations" class="w-full h-full object-contain" />
        </transition>
      </a>
    </div>
    ...
    <ul class="hidden md:flex space-x-12 items-center flex-1 justify-center">
      <li v-for="link in navLinks" :key="link.name">
        <a :href="link.url"
          ...
          {{ link.name }}
          <span
            class="absolute -bottom-1 left-0 w-0 h-px bg-stone-900 transition-all duration-300 group-hover:w-full"></span>
        </a>
      </li>
    </ul>

    <div class="flex-1 flex justify-end items-center space-x-2 md:space-x-8">
      <a href="https://wa.me/923352210993" target="_blank" rel="noopener noreferrer"
        class="relative p-2 hover:bg-green-50 rounded-full transition-colors group" title="Contact on WhatsApp">
        <svg class="w-5 h-5 md:w-6 md:h-6 text-green-600 hover:text-green-700" fill="currentColor" viewBox="0 0 24 24">
          ...
          <path
            d="M12 0C5.373 0 0 5.373 0 12s5.373 12 12 12 12-5.373 12-12S18.627 0 12 0m0 22C6.477 22 2 17.523 2 12S6.477 2 12 2s10 4.477 10 10-4.477 10-10 10" />
        </svg>
      </a>

      <button @click="mobileMenuOpen = !mobileMenuOpen"
        class="md:hidden text-stone-900 p-2 hover:bg-stone-100 rounded-lg transition-colors">
    ...
    <transition name="fade">
      <div v-if="mobileMenuOpen"
        class="fixed inset-0 top-[64px] md:top-[73px] bg-white/95 backdrop-blur-sm z-[60] flex flex-col p-6 md:hidden shadow-lg">
        <div class="flex flex-col space-y-4">
          <a v-for="link in navLinks" :key="link.name" :href="link.url" @click="mobileMenuOpen = false"
          ...
          <a href="https://wa.me/923352210993" target="_blank" rel="noopener noreferrer" @click="mobileMenuOpen = false"
